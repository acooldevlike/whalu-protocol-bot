...
exports.freezeStatusCommand = freezeStatusCommand;
const db_1 = __importDefault(require("../../database/db"));
const freeze_1 = require("../../services/freeze");
async function freezeStatusCommand(ctx) {
    try {
        // Check database for freeze record
        const freezeRecord = db_1.default.prepare('SELECT * FROM freeze_protocol WHERE frozen = 1 ORDER BY freeze_date DESC LIMIT 1').get();
        const devWallet = process.env.DEV_WALLET_ADDRESS;
        const devTokenAccount = process.env.DEV_TOKEN_ACCOUNT;
        if (!devWallet) {
            await ctx.reply(`💎 *FREEZE PROTOCOL STATUS*\n\n` +
                `Status: ⚠️ Not configured\n\n` +
                `Developer wallet not set.`, { parse_mode: 'Markdown' });
            return;
        }
        ...
        if (freezeRecord) {
            ...
            if (devTokenAccount) {
                ...
                const onChainStatus = await (0, freeze_1.checkFreezeStatus)(devTokenAccount);
        ...
        await ctx.reply(`❌ Error checking freeze status.\n\n` +
            `Please try again later.`);
    }
}
//# sourceMappingURL=freezeStatus.js.map