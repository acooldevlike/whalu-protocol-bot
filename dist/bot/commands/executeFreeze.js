...
exports.executeFreezeCommand = executeFreezeCommand;
exports.handleFreezeConfirmation = handleFreezeConfirmation;
const db_1 = __importDefault(require("../../database/db"));
const freeze_1 = require("../../services/freeze");
// Session state management (simple in-memory store)
const freezeConfirmationSessions = new Map();
async function executeFreezeCommand(ctx) {
    ...
    const existingFreeze = db_1.default.prepare('SELECT * FROM freeze_protocol WHERE frozen = 1 LIMIT 1').get();
    if (existingFreeze) {
        await ctx.reply(`⚠️ Freeze Protocol already executed!\n\n` +
    ...
    if (!process.env.DEV_WALLET_ADDRESS || !process.env.DEV_TOKEN_ACCOUNT || !process.env.DEV_WALLET_PRIVATE_KEY) {
        await ctx.reply(`❌ Configuration Error\n\n` +
    ...
    await ctx.reply(`⚠️ *WARNING: Execute Freeze Protocol?*\n\n` +
        ...
        `Type anything else to cancel.`, { parse_mode: 'Markdown' });
}
async function handleFreezeConfirmation(ctx) {
    const telegramId = ctx.from.id;
    ...
    await ctx.reply(`🔒 *Executing Freeze Protocol...*\n\n` +
        `This will take a moment. The whale is locking down... 🐋💎`, { parse_mode: 'Markdown' });
    try {
        // Execute freeze
        ...
        const signature = await (0, freeze_1.freezeDevWallet)(devWalletPrivateKey, devTokenAccount);
        // Verify freeze was successful
        console.log('🔍 Verifying freeze on-chain...');
        const isVerified = await (0, freeze_1.verifyFreeze)(devTokenAccount);
        if (!isVerified) {
            throw new Error('Freeze verification failed - wallet may not be properly frozen');
        }
        // Update database
        const freezeDate = Math.floor(Date.now() / 1000);
        db_1.default.prepare(`INSERT INTO freeze_protocol (wallet_address, frozen, freeze_date, freeze_type, tx_signature) 
       VALUES (?, ?, ?, ?, ?)`).run(devWalletAddress, 1, freezeDate, 'permanent', signature);
        const txLink = process.env.SOLANA_RPC_URL?.includes('devnet')
            ? `https://solscan.io/tx/${signature}?cluster=devnet`
            : `https://solscan.io/tx/${signature}`;
        await ctx.reply(`✅ *FREEZE PROTOCOL EXECUTED*\n\n` +
            ...
            `Use /freeze_status to view public status.`, { parse_mode: 'Markdown' });
        // Optional: Broadcast to all users
        await broadcastFreezeAnnouncement(signature);
    }
    catch (error) {
        console.error('❌ Freeze execution failed:', error);
        await ctx.reply(`❌ *Freeze Failed*\n\n` +
            `Error: ${error.message}\n\n` +
            `The wallet was NOT frozen. Please check logs and try again.`, { parse_mode: 'Markdown' });
    }
    return true;
}
async function broadcastFreezeAnnouncement(signature) {
    try {
...
//# sourceMappingURL=executeFreeze.js.map